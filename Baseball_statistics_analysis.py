...
def filter_by_year(statistics, year, yearid):
    ...
    """
    return list(filter(lambda stat: stat[yearid] == str(year), statistics))

def formula(info, batting_stats):
    """
    Computes a simple average of batting average, on-base percentage,
    and slugging percentage.
    """
    return (batting_average(info, batting_stats) +
            onbase_percentage(info, batting_stats) +
            slugging_percentage(info, batting_stats)) / 3
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for player_stats in statistics:
        ...
        compound_stat = formula(info, player_stats)
        results.append((player_id, compound_stat))
    results.sort(key=lambda x: x[1], reverse=True)
    return results[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    master_data = read_csv_as_nested_dict(info["masterfile"],
                                          info["playerid"],
                                          info["separator"],
                                          info["quote"])
    newlist = []
    for player_id, stat in top_ids_and_stats:
        player_info = master_data[player_id]
        newlist.append(f"{stat:.3f} --- {player_info[info['firstname']]} {player_info[info['lastname']]}")
    return newlist
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics:
        ...
        if pid not in aggregated:
            ...
            for field in fields:
                aggregated[pid][field] = 0
        for field in fields:
            aggregated[pid][field] += int(row[field])
    return aggregated
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    results = []
    for pid, player_stats in aggregate_by_player_id(year_stats,info["playerid"], info["battingfields"]).items():
        stat_value = formula(info, player_stats)
        results.append((pid, stat_value))
    results.sort(key=lambda x: x[1], reverse=True)
    return lookup_player_names(info, results[:numplayers])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    statistics = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])
    aggregated = aggregate_by_player_id(statistics, info["playerid"], info["battingfields"])
    results = []
    for pid, player_stats in aggregated.items():
        stat_value = formula(info, player_stats)
        results.append((pid, stat_value))
    results.sort(key=lambda x: x[1], reverse=True)
    return lookup_player_names(info, results[:numplayers])
...
def test_baseball_statistics():
    ...
    top_ops_2010 = compute_top_stats_year(baseballdatainfo, lambda info, stats: (onbase_percentage(info, stats) + slugging_percentage(info, stats)), 10, 2010)
...
# Make sure to comment this line before submitting to Coursera/OwlTest
